...
export const maxDuration = 300; // up to 5 min jobs

// Optional env overrides (absolute paths recommended)
...
export async function POST(req: NextRequest) {
  try {
    ...
    const bitrate = form.get("bitrate") ? String(form.get("bitrate")) : undefined; // kbps for mp3/aac
    ...
    if (!FFMPEG_PATH || FFMPEG_PATH === "null") {
      return bad(500, "ffmpeg not found. Install ffmpeg or set FFMPEG_PATH.");
    }
    ...
    // normalization (simple single-pass EBU R128)
    ...
    switch (fmt) {
      case "mp3":
        ...
        args.push("-b:a", `${Number(bitrate || "192")}k`);
        break;
      case "aac":
        args.push("-c:a", "aac");
        args.push("-b:a", `${Number(bitrate || "192")}k`);
      ...
      case "flac":
        ...
        break;
      case "wav":
        args.push("-c:a", "pcm_s16le"); // 16-bit WAV
        break;
    }
